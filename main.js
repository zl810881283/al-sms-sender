...
Observable.from(users)
  ...
  .map(i => {
    // ${name}，今天${localtion}的空气质量指数为${aqi}(${aqi_level})，建议${window_open}门窗，${outdoor_advise}户外活动，${use_mask}。
    let templateParam = {
      name: i.name,
      localtion: i.city,
      aqi: i.aqi,
    ...
    if (i.aqi <= 50) {
      ...
      templateParam.use_mask='请务必佩戴口罩'
    } else {
